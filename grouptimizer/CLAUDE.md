...
5. PWA bits: `manifest.webmanifest` (name/icons/standalone), `sw.js` (network-
   first offline shell for same-origin files only), and the `icon-*.png` /
   `favicon-32.png` files, generated by a throwaway pure-stdlib Python script
   (no image libs on this machine). Installability needs HTTPS or localhost.
...
**2026-09-21 installable PWA** — Added `manifest.webmanifest`, `sw.js`, and
any/maskable PNG icons (three dot clusters on the app's `#222`) so Add to Home
Screen on Android gets a real icon and a standalone window. The service worker is
deliberately network-first and same-origin-only, so app edits show up right away
and the Google Sheet fetch is never cached. Bump `CACHE` in `sw.js` if the file
list changes.
