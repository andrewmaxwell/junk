...
export const getData = async () => {
  ...
  const data = window.Papa.parse(text, {header: true})
    .data.map((row) => ({
      ...
      energy: row['Energy Level'] === '' ? NaN : +row['Energy Level'],
      anxiety: row['Anxiety Level'] === '' ? NaN : +row['Anxiety Level'],
      headache: row.Headache === '' ? NaN : +row.Headache,
      mood: row.Mood === '' ? NaN : +row.Mood,
      exercise: row.Exercise === '' ? NaN : +row.Exercise,
      notes: row.Notes,
      temperature: 0,
      precipitation: 0,
      pressure: 0,
