const parse = input =>
  input
    .split('\n')
    .map(r => r.split(',').map(c => ({dir: c[0], dist: Number(c.slice(1))})));
...
const getCoords = wire => {
  ...
  for (let i = 0; i < wire.length; i++) {
    const dir = dirs[wire[i].dir];
    for (let j = 0; j < wire[i].dist; j++) {
      c[0] += dir.x;
      c[1] += dir.y;
      res.push(c.join(','));
      // res.push(c.slice());
...
const intersection = (a, b) => {
  const aIndex = a.reduce((res, c) => {
    res[c] = true;
    return res;
  }, {});
  return b.filter(c => aIndex[c]);
};

const closestCrossing = wires => {
  const [a, b] = wires.map(getCoords);
  return Math.min(
    ...intersection(a, b).map(
      c => a.indexOf(c) + b.indexOf(c) + 2
...
console.log(closestCrossing(parse(input)));
