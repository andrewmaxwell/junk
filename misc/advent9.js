class Computer {
  constructor(data, onOutput) {
    this.origData = data.split(',').map(Number);
    this.onOutput = onOutput;
    this.reset();
  }
  reset() {
    this.data = this.origData.slice();
    this.inputs = [];
    this.instCounter = 0;
    this.relativeBase = 0;
    return this;
  ...
  run(...vals) {
    ...
    while (!this.isDone()) {
      ...
      const addresses = args.map((v, i) =>
        modes[i] === 2 ? v + this.relativeBase : v
      );

      const [va, vb] = args.map((v, i) =>
        modes[i] === 1 ? v : data[addresses[i]] || 0
      );

      const [aa, , ac] = addresses;
      ...
      switch (op) {
        ...
        case 8:
          ...
          break;
        case 9:
          this.relativeBase += va;
          this.instCounter += 2;
          break;
        case 99:
          return;
        default:
          throw new Error(`Bad op: ${JSON.stringify([opCode, ...args])}`);
      }
    }
  }
}
...
// new Computer(program, x => console.log('output', x)).run(2);
