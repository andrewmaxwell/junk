// const Computer = require('./advent9.js');
import Computer from './advent9.js';
...
const output = [];
const computer = new Computer(program, x => output.push(x));
...
const pathToClosest = (pos, grid, goal) => {
  const q = [pos];
  for (let i = 0; i < q.length; i++) {
    if (grid[q[i].y][q[i].x] === goal) {
      let path = [];
      while (q[i].prev) {
        path.push(q[i].command);
        q[i] = q[i].prev;
      }
      return path.reverse();
    }
    dirs.forEach(({x, y, command}) => {
      const nx = q[i].x + x;
      const ny = q[i].y + y;
      if (grid[ny][nx] !== '#' && !q.some(o => o.x === nx && o.y === ny)) {
        q.push({x: nx, y: ny, prev: q[i], command});
      }
    });
  }
};
...
const loop = () => {
  window.pre.innerHTML = grid
    .map((r, y) =>
      r
        .map((c, x) =>
          y === pos.y && x === pos.x
            ? 'D'
            : goal && goal.y === y && goal.x === x
            ? 'G'
            : c
        )
        .join('')
  ...
  const path = pathToClosest(pos, grid, ' ');

  if (!path) {
    grid[goal.y][goal.x] = 'G';
    console.log(pathToClosest({x: size / 2, y: size / 2}, grid, 'G'));
    return;
  }
  path.forEach(cmd => {
    computer.run(cmd);
    ...
    const dir = dirs.find(d => d.command === cmd);
