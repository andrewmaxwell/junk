...
(function(exports) {
  ...
  var _expect = function(passed, msg, options) {
    ...
    if (passed) {
      ...
      write(
        logFilter('<div class="console-passed">') +
          successMsg +
          logFilter('</div>', true),
        true
      );
      ...
      if (options.extraCredit) {
        msg =
          options.extraCredit !== true ? _message(options.extraCredit) : null;
        msg = combineMessages(['Test Missed', msg], ': ');
        write(
          logFilter("<div class='console-missed'>") +
            msg +
            logFilter('</div>', true),
          true
        );
        ...
        write(
          logFilter("<div class='console-failed'>Test Failed: ") +
            msg +
            logFilter('</div>', true),
          true
        );
  ...
  var Test = {
    ...
    expect(passed, message, options) {
      logCall('expect');
      _expect(passed, message, options);
    },
    assertSimilar(actual, expected, msg, options) {
    ...
    assertNotSimilar(actual, expected, msg, options) {
    ...
    assertNotEquals(a, b, msg, options) {
      ...
      if (a === b) {
        ...
        options = options || {};
        options.successMsg =
          options.successMsg || 'Value != ' + Test.inspect(b);
        Test.expect(true, null, options);
      }
    },
    expectNoError(msg, fn) {
    ...
    expectError(msg, fn, options) {
      ...
      try {
        ...
        console.log(
          logFilter('<b>Expected error was thrown:</b> ') + ex.toString()
        );
        passed = true;
      }
