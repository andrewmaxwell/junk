export class Display {
  ...
  resize() {
    this.width = this.topCanvas.width = this.topCanvas.height = Math.min(
      window.innerWidth,
      window.innerHeight
    );
    this.bottomCanvas.style.width = this.bottomCanvas.style.height =
      this.width + 'px';
  }
  renderFluid(fluid) {
    ...
    if (this.rows != rows) {
      ...
      for (i = 3; i < this.imageData.data.length; i += 4) {
