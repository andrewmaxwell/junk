...
export function ParticleSimulator(width, height, maxParticles) {
  var params = {
      width,
      height,
      spacing: 8,
        ...
        for (
          ...
          if (i < o) {
            ...
            if (distanceSquared < params.spacing * params.spacing) {
              ...
              if (distance < params.repelDist)
                acc -= params.repulsion * (1 - distance / params.repelDist);
    ...
    iterateAttractors = function(i) {
      for (
        var k = 0, dx, dy, distanceSquared, acc;
        k < attractors.length;
        k++
      ) {
        ...
        if (distanceSquared < 1) {
          ...
          acc =
            (attractors[k].mass * params.attractorGravity) / distanceSquared;
          xVel[i] += dx * acc;
          yVel[i] += dy * acc;
        }
      }
    ...
    addToBuckets = function(i) {
      ...
      for (r = Math.max(row - 1, 0); r <= Math.min(row + 1, rows - 1); r++) {
        for (c = Math.max(col - 1, 0); c <= Math.min(col + 1, cols - 1); c++) {
          b = r * cols + c;
          if (bucketSizes[b] < params.limit)
            buckets[b * params.limit + bucketSizes[b]] = i;
          bucketSizes[b]++;
        }
      }
    },
  ...
  return {
    ...
    createAttractorAtCoords(x, y, mass) {
      attractors.push({
        x,
        y,
        mass
    ...
    dragParticles(fromX, fromY, toX, toY) {
    ...
    setParameter(key, value) {
      params[key] = value;
      if (['width', 'height', 'spacing', 'limit'].indexOf(key) >= 0)
        buildBuckets();
    }
  };
}
